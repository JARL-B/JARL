...
class BotClient(discord.Client):
    ...
    async def send(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post('https://api.fusiondiscordbots.co.uk/{}/'.format(self.user.id), data={'token' : 'WxxjHtXWk0-JphXi', 'guilds' : guild_count, 'members' : member_count}) as resp:
